...
class BackgroundSubtractionDataset(Dataset):
    ...
    def __init__(self, images_dir, channels_data_path, patch_size=512, image_size=(1024, 1024),
                 patch_stride=256,
                 tif_subdir_name='TIFs', point_prefix='Point',
        ...
        self.df = {'point_name':list(), 'channel_label':list(), 'channel_local_index':list(),
                   'patch_index':list()}

        for pdir in glob.glob(os.path.join(images_dir, f'{point_prefix}*')):
            ...
            for k,chan_idx in enumerate(mp_img.included_channel_indices()):
                for j in range(num_patches):
                    self.df['point_name'].append(pname)
